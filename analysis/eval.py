...
def ensure_output_layout(output_root: Path) -> dict[str, Path]:
	paths = {
		...
		"paper_main": output_root / "paper" / "main_text",
		"paper_appendix": output_root / "paper" / "appendix",
	}
...
def format_ci(low: float, high: float, digits: int = 4) -> str:
...
def get_experiment_group(experiment_name: str) -> str:
	if not experiment_name:
		return "Other"
	prefix = experiment_name[0].upper()
	if prefix in {"B", "C"}:
		return prefix
	return "Other"
...
def write_markdown_table(path: Path, header: list[str], rows: list[list[Any]]) -> None:
...
def write_latex_table(path: Path, rows: list[list[str]]) -> None:
	with path.open("w", encoding="utf-8") as f:
		f.write("% Auto-generated by analysis/eval.py\n")
		f.write("\\begin{table}[t]\n")
		f.write("\\centering\n")
		f.write("\\caption{Main comparison across experiments (mean$\\pm$std over folds).}\n")
		f.write("\\label{tab:main_comparison}\n")
		f.write("\\begin{tabular}{lccc}\n")
		f.write("\\hline\n")
		f.write("Experiment & Accuracy & F1-macro & Balanced Acc. \\\\\n")
		f.write("\\hline\n")
		for row in rows:
...
def write_latex_table_with_caption(
	path: Path,
	rows: list[list[str]],
	caption: str,
	label: str,
...
def write_paper_highlights(
	...
	with path.open("w", encoding="utf-8") as f:
		...
		f.write(
			f"- Top experiment: **{top['experiment']}**\\n"
			f"- Accuracy: **{format_pm(top['accuracy_mean'], top['accuracy_std'])}**\\n"
			f"- F1-macro: **{format_pm(top['f1_macro_mean'], top['f1_macro_std'])}**\\n"
			f"- Balanced accuracy: **{format_pm(top['balanced_accuracy_mean'], top['balanced_accuracy_std'])}**\\n"
		)

		f.write("\n## Statistical Comparison (vs Reference)\n")
		f.write(f"- Reference experiment: **{reference_name}**\n")
		f.write("- Significance rule: p < 0.05\n\n")

		md_rows: list[list[str]] = []
		for row in sig_rows:
			experiment, _, n_common, delta_f1, delta_acc, p_f1, p_acc = row
			md_rows.append([
				experiment,
				str(n_common),
				f"{delta_f1:+.4f}",
				f"{delta_acc:+.4f}",
				format_pvalue(float(p_f1)),
				format_pvalue(float(p_acc)),
			])

		f.write("| experiment | n_common_folds | Δf1_macro | Δaccuracy | p(f1) | p(acc) |\n")
		f.write("|---|---:|---:|---:|---:|---:|\n")
		for row in md_rows:
			f.write("| " + " | ".join(row) + " |\n")
		...
		f.write(
			...
			f"**{top['experiment']}** with "
			f"Accuracy={top['accuracy_mean']:.4f}±{top['accuracy_std']:.4f}, "
			f"F1-macro={top['f1_macro_mean']:.4f}±{top['f1_macro_std']:.4f}, and "
			f"Balanced Accuracy={top['balanced_accuracy_mean']:.4f}±{top['balanced_accuracy_std']:.4f}.\n"
		)
...
def evaluate_experiment(experiment_name: str, run_dir: Path, experiments_output_dir: Path) -> dict:
	...
	exp_output = experiments_output_dir / experiment_name
...
def main() -> None:
	...
	output_paths = ensure_output_layout(output_root)
	...
	for exp_dir in experiment_dirs:
		...
		summary = evaluate_experiment(exp_dir.name, run_dir, output_paths["experiments"])
	...
	write_csv(
		output_paths["summary"] / "significance_vs_reference.csv",
		[
			"experiment",
			"reference",
			"n_common_folds",
	...
	write_csv(output_paths["summary"] / "experiment_comparison_summary.csv", summary_header, summary_rows)
	with (output_paths["summary"] / "experiment_comparison_summary.json").open("w", encoding="utf-8") as f:
	...
	for row in all_summaries:
		paper_rows.append([
			row["experiment"],
			format_pm(row["accuracy_mean"], row["accuracy_std"]),
			format_ci(row["accuracy_ci95_low"], row["accuracy_ci95_high"]),
			format_pm(row["f1_macro_mean"], row["f1_macro_std"]),
			format_ci(row["f1_macro_ci95_low"], row["f1_macro_ci95_high"]),
			format_pm(row["balanced_accuracy_mean"], row["balanced_accuracy_std"]),
		])
	write_csv(
		output_paths["paper"] / "paper_table.csv",
		[
			"experiment",
			"accuracy_mean±std",
			"accuracy_95ci",
	...
	write_markdown_table(
		output_paths["paper"] / "paper_table.md",
		[
			"experiment",
			"accuracy (mean±std)",
			"accuracy 95% CI",
	...
	write_latex_table(output_paths["paper"] / "paper_table.tex", latex_rows)

	write_markdown_table(
		output_paths["paper"] / "significance_vs_reference.md",
		[
			"experiment",
			"reference",
			"n_common_folds",
		...
		[
			[
				...
				f"{float(r[3]):+.4f}",
				f"{float(r[4]):+.4f}",
				format_pvalue(float(r[5])),
				format_pvalue(float(r[6])),
			]
			for r in sig_rows
		],
	)

	write_paper_highlights(
		output_paths["paper"] / "paper_highlights.md",
		all_summaries,
		reference_name,
		sig_rows,
	)
	...
	grouped = {"B": [], "C": [], "Other": []}
	for row in all_summaries:
		grouped[get_experiment_group(row["experiment"])].append(row)

	def rows_for_group(group_rows: list[dict]) -> list[list[str]]:
		return [
			[
				r["experiment"],
				format_pm(r["accuracy_mean"], r["accuracy_std"]),
				format_pm(r["f1_macro_mean"], r["f1_macro_std"]),
				format_pm(r["balanced_accuracy_mean"], r["balanced_accuracy_std"]),
			]
			for r in group_rows
		]

	# Main text: default emphasize C-group (fusion comparison)
	main_rows = rows_for_group(grouped["C"] if grouped["C"] else all_summaries)
	write_markdown_table(
		output_paths["paper_main"] / "table_main.md",
		main_header,
		main_rows,
	)
	write_latex_table_with_caption(
		output_paths["paper_main"] / "table_main.tex",
		main_rows,
		"Main-text comparison table (default: C-group fusion experiments).",
		"tab:main_text_comparison",
	)

	# Appendix: full and grouped tables
	appendix_all_rows = rows_for_group(all_summaries)
	appendix_b_rows = rows_for_group(grouped["B"])
	appendix_c_rows = rows_for_group(grouped["C"])

	write_markdown_table(
		output_paths["paper_appendix"] / "table_appendix_all.md",
		main_header,
		appendix_all_rows,
	)
	write_markdown_table(
		output_paths["paper_appendix"] / "table_appendix_B_group.md",
		main_header,
		appendix_b_rows,
	)
	write_markdown_table(
		output_paths["paper_appendix"] / "table_appendix_C_group.md",
		main_header,
		appendix_c_rows,
	)

	write_latex_table_with_caption(
		output_paths["paper_appendix"] / "table_appendix_all.tex",
		appendix_all_rows,
		"Appendix table: full comparison across all experiments.",
		"tab:appendix_all",
	)
	write_latex_table_with_caption(
		output_paths["paper_appendix"] / "table_appendix_B_group.tex",
		appendix_b_rows,
		"Appendix table: B-group ablation comparison.",
		"tab:appendix_B",
	)
	write_latex_table_with_caption(
		output_paths["paper_appendix"] / "table_appendix_C_group.tex",
		appendix_c_rows,
		"Appendix table: C-group fusion comparison.",
		"tab:appendix_C",
	)
	...
	print(f"- per-experiment details: {output_paths['experiments']}")
	print(f"- global summary: {output_paths['summary']}")
	print(f"- paper-ready outputs: {output_paths['paper']}")
	print(f"  - main-text tables: {output_paths['paper_main']}")
	print(f"  - appendix tables: {output_paths['paper_appendix']}")
