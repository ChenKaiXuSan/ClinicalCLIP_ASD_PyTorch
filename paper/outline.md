...
3.1 **Attention as supervision(对照)**:concept 架构(5 概念、存在性 + grounding 损失),推理只用视频;
    attn_align 0.537 / Grad-CAM 0.135 / 均匀 0.068;去 grounding 0.118。
...
| 方法 | 输入 | 患者级 macro | AUC |
|---|---|---|---|
| 3D CNN(slow_r50) | 视频 | 0.644±0.045 | 0.70 |
| + region grounding(M0) | 视频 | 0.663±0.030 | 0.60 |
| ST-GCN 2D / 3D 骨架 | 骨架 | 0.513±0.010 / 0.538±0.026 | 0.55 |
| 随机森林 / 梯度提升,1272 量 | 3D 测量 | 0.73 / 0.76(79 人,固定划分) | |
...
4.4 **消融**(Table 3):5 量留一(几何 / 门控):去头前伸 0.740 / 0.729,去躯干前倾 0.727 / 0.754,其余 0.75–0.80;
    门控对照:置换、未标部位、嵌套 w、sigmoid;几何分支强(≥0.79)时门控收益归零(如实写)。
